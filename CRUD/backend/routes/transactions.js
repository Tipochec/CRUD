...
router.get('/', (req, res) => { //Клиент делает GET запрос на /api/transactions
  Transaction.getAll((err, rows) => { // с этого сайта берётся всё, функция ждёт ответа от Базы данных
    if (err) { // если ошибка
...
router.post('/', (req, res) => { // POST отвечает за отправку данных
  ...
  Transaction.create({ amount, category_id, type, description, category_name   }, function(err) { // создание транзакции, берём те же поля из объекта.
...
router.delete('/:id', (req, res) => { // DELETE отвечает за удаление операции, сортировка идёт по id, так как это самый удобный способ удалить данные, id всегда уникален.
  const id = req.params.id; // получаем ID из URL Пример: DELETE /api/transactions/5 → req.params.id = "5"
  Transaction.delete(id, function(err) { // удаление операции по её id
...
router.get('/stats', (req, res) => { // GET — полуаем данные, по такой то ссылке
  Transaction.getStats((err, stats) => { // получаем все данные.
    if (err) { // Если ошибка
      res.status(500).json({ error: err.message }); // выводим её
